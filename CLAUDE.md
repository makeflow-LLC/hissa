...
### Interactive activities: one content set, twelve games
...
Twelve kinds: `pyramid` · `labeling` · `match` · `flashcards` · `quiz` · `anagram` · `sort` · `memory` · `truefalse` · `balloons` · `speed` · `wheel`. Each is one file under `components/games/`, and `ActivityPlayer` is the shell that dispatches and records. A game's only outlet is `onFinish(score, total)` — it knows nothing about the database, so a thirteenth kind is one file, one line in the dispatch, one entry in `KINDS`, and one widened `check` constraint. `readKind()` in `app/actions/activities.ts` derives from `KINDS`, so a new kind cannot be silently rejected by the server action.
...
**`labeling` (سمِّ الأجزاء) is the first kind that needs one image for the whole activity**, so `activities.image_url` is a real column while each label's position (`x`, `y`) rides inside its item in `items`. Positions are stored as **percentages**, not pixels, so they survive every screen size.

Two things about it are easy to get wrong and were:

- **Dragging uses pointer events, never HTML5 drag-and-drop.** `dragstart/drop` does nothing at all on touch screens, and this audience is overwhelmingly on phones — the game would have looked simply broken to most students. A **tap-then-tap** path exists alongside it, because precise dragging fails often on small fingers and slow devices and one path only would lock some students out entirely.
- **The save path must forward the whole item, not just `{a, b}`.** `readItems` rebuilt each row from the two text fields alone, dropping `img`, `x` and `y` before `cleanItems` could validate them — so activities saved with no images and no positions, and both games rendered empty of them. Validation still happens in `cleanItems` (`safeImageUrl`, `pct`), so forwarding the raw fields opens nothing.
- **The image's frame is sized from the image's aspect ratio, never by shrink-to-fit** (`components/ImageFrame.tsx`). Percent coordinates only mean something if the positioning box *is* the rendered image box, and the obvious ways to get that — `width: fit-content` or `display: inline-block` on a wrapper holding an image with `max-width: 100%` — are **circular**: the wrapper's width derives from the image and the image's width derives from the wrapper. Inside a flex column Chrome resolves that cycle to **zero**, so the image vanished, the teacher's clicks landed on nothing, no coordinates were ever saved, and every point fell back to the 50%/50% default — which is why they all stacked in the centre. The frame now reads `naturalWidth/naturalHeight` on load and sets `aspect-ratio` plus `width: min(100%, ratio × maxVh)`, all definite. It reads the ratio from a `ref` callback **as well as** `onLoad`, because a cached image completes before React attaches the listener and `onLoad` then never fires.
- **Empty points carry no number.** They used to show the item's row number, which is meaningless to the student and appears scrambled (1, then 3, then 4) as points fill — read as a deliberate order the student then tries to follow. They are plain pins now; the name appears only once placed.
- **Matching is by name, not row index.** A teacher who names two parts the same («ورقة» and «ورقة») had a correct drop rejected because the indices differed — an error the student can neither understand nor get past. Names are compared through `normalizeArabic`.
- **The accept radius comes from the spacing between points, not a fixed floor.** A 56px minimum meant two nearby points shared one radius, so a drop snapped to "the nearest" rather than the one aimed at — which is exactly what reads as imprecise matching. Each point now gets half the distance to its closest neighbour (clamped 26–90px), so radii never overlap, and ties break on distance *relative* to each radius.
- **A wrong drop costs something.** Wrong drops are refused, so the game handed out full marks every time and the score meant nothing; misses now cost a third of a point each, floored at 1.
- **Hit-testing is in pixels, and the image must share the screen with the tray.** Comparing distance in *percent* mixes two scales — 12% of a wide image is far more pixels horizontally than vertically — so a drop lands sometimes and misses sometimes for no visible reason. And a tall image pushed the label tray below the fold: you cannot drag across a scroll, so the game felt broken rather than imprecise. The image is capped at `50vh` and the tray is `position: sticky`, so both stay on one screen. Coordinates are measured against the **image** rect, not the bordered wrapper, whose 1px edge shifts the origin.
- **Dot positions use `left`, not `inset-inline-start`.** The platform is RTL, so the logical property mirrors the point: the teacher clicks the right of the image and the dot lands on the left, and no student drag ever hits a target because the stored coordinate is measured from the left while the render measures from the right. A position on a picture is not text and does not follow reading direction. A browser drag test caught this — the tap path passed while dragging failed 100% of the time.
...
**Removed: summarizing from an uploaded PDF.** A teacher could upload the lesson as a PDF and have the model read it. It was taken out at the owner's request after it failed in production; the summary tool now works only on what is in the editor. Two findings are worth keeping if it ever returns: extracting the text first is **wrong for Arabic** (pdf.js returns Presentation Forms in *visual* order — «الدرس الثالث: قانون أوم» comes back as «أوم نﻮﻧﺎﻗ :ﺚﻟﺎﺜﻟا رسﺪﻟا», and NFKC fixes the letterforms but not the order), so the file itself must go to the model; and `pdf.js` **detaches the buffer it is given**, so counting pages before base64-encoding the same `Uint8Array` yields an empty string and OpenRouter answers `400 Invalid base64 data URL` — hand it a copy.
...
`LessonDesigner` has an optional **مادّة مرجعية** box (6000 chars) beside the free-text توصيات. Pasting a passage from the prescribed textbook makes the model take its definitions, examples, terms and figures verbatim rather than substituting its own general knowledge; anything the passage omits is filled in to suit the stage without contradicting it. The rule is only added to the system prompt when the field is non-empty (`hasRefs`), so a blank box does not tell the model to obey a reference that isn't there.
